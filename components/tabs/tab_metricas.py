...
from ..field import TEAM_COLORS, TEAM_NAMES, FIELD_W, FIELD_H


def calcular_metricas(frames, player_team):
    """Calcula todas las métricas directamente desde los frames."""
    ...
    for frame in frames:
        ...
        # Presión (distancia entre centroides)
        if c0 is not None and c1 is not None:
            presion = abs(c0 - c1)
            presion_sum += presion
            presion_count += 1
        ...
        por_frame.append({
            ...
            "e0": {
                "centroide_x": round(c0, 2) if c0 else None,
                "profundidad": round(compact_sums["e0"] / max(compact_counts["e0"], 1), 2),
            },
            "e1": {
                "centroide_x": round(c1, 2) if c1 else None,
                "profundidad": round(compact_sums["e1"] / max(compact_counts["e1"], 1), 2),
            },
            "distancia_entre_equipos": round(presion, 2) if c0 and c1 else None,
        })
    ...
    for eq_key in ["e0", "e1"]:
        ...
        resumen["velocidad_media_ms"][eq_key] = round(
            vel_sums[eq_key] / max(vel_counts[eq_key], 1), 2
        )
        resumen["compacidad_media_m"][eq_key] = round(
            compact_sums[eq_key] / max(compact_counts[eq_key], 1), 1
        )
        resumen["posicion_media_x"][eq_key] = round(
            pos_sums[eq_key] / max(pos_counts[eq_key], 1), 1
        )

    total_territorial = territorial_e0 + territorial_e1 or 1
    resumen["posesion_territorial_pct"]["e0"] = round(territorial_e0/total_territorial*100)
    resumen["posesion_territorial_pct"]["e1"] = round(territorial_e1/total_territorial*100)
    ...
    # Distribución por zonas (defensa / medio / ataque)
    for eq_key, team_id in [("e0", 0), ("e1", 1)]:
        zona_counts = {"defensa": 0, "medio": 0, "ataque": 0}
        for frame in frames:
            for det in get_jugadores(frame):
                if not isinstance(det, dict): continue
                if get_team(det) != team_id: continue
                x, _ = get_pos(det)
                if x < FIELD_W/3:
                    zona_counts["defensa"] += 1
                elif x < 2*FIELD_W/3:
                    zona_counts["medio"] += 1
                else:
                    zona_counts["ataque"] += 1
        total = sum(zona_counts.values()) or 1
        resumen.setdefault("distribucion_zonas_pct", {})[eq_key] = {
            z: round(v/total*100) for z, v in zona_counts.items()
        }
...
def render(frames, player_team):
    ...
    with c3: st.metric("Presión media",
                        f"{res['presion_media_m']} m",
                        help="Distancia media entre centroides. Menor = más presión.")
    ...
    tab_c, tab_co, tab_p = st.tabs(
        ["Posición en campo", "Compacidad", "Presión"]
    )
